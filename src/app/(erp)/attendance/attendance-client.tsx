      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        {allDepts.length > 1 && (
          <div className="ml-2 flex flex-wrap gap-1">
            <button onClick={() => setDept("")} className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${dept === "" ? "bg-indigo-500 text-white" : "border border-neutral-300 bg-white text-neutral-600 hover:bg-neutral-50"}`}>전체</button>
            {allDepts.map((d) => (
              <button key={d} onClick={() => setDept(d)} className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${dept === d ? "bg-indigo-500 text-white" : "border border-neutral-300 bg-white text-neutral-600 hover:bg-neutral-50"}`}>{d}</button>
            ))}
          </div>
        )}
      </div>

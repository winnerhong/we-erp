  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          canCreate ? (
            <button onClick={() => openNew()} className="rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-400">
              + 새 업무
            </button>
          ) : null
        }
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex rounded-lg border border-neutral-200 bg-white p-1 text-sm">
          {([["month", "📅 캘린더"], ["timeline", "👥 타임라인"], ["kanban", "✅ 칸반"]] as const).map(([k, lbl]) => (
            <button key={k} onClick={() => setView(k)} className={`rounded-md px-3 py-1.5 font-medium ${view === k ? "bg-indigo-500 text-white" : "text-neutral-600 hover:bg-neutral-100"}`}>{lbl}</button>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-2 rounded-xl border border-neutral-200 bg-white p-3">
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {showMine && (
          <button onClick={onMine} className={`ml-2 rounded-full border px-2.5 py-1 text-xs font-medium ${mineOnly ? "border-neutral-900 bg-indigo-500 text-white" : "border-neutral-300 text-neutral-600 hover:bg-neutral-50"}`}>
            ⭐ 내 업무만
          </button>
        )}
        {active > 0 && (
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {employees.map((e) => (
          <button key={e.id} onClick={() => onEmp(e.id)}
            className={`inline-flex items-center gap-1 rounded-full border py-0.5 pl-0.5 pr-2 text-xs font-medium ${fEmp.has(e.id) ? "border-neutral-900 bg-indigo-500 text-white" : "border-neutral-300 text-neutral-600 hover:bg-neutral-50"}`}>
        ...
        <div className="flex border-b border-neutral-100">
          ...
          <div className="flex">
            {days.map((d) => {
              const wd = dowOf(d);
              return (
                <div key={d} style={{ width: cellW }} className={`border-r border-neutral-50 py-1 text-center text-[10px] tabular-nums ${d === today ? "bg-indigo-500 font-bold text-white" : wd === 0 ? "text-rose-400" : wd === 6 ? "text-blue-400" : "text-neutral-400"}`}>
              ...
              {companyEmployees.map((e) => {
                ...
                return (
                  <button key={e.id} type="button" disabled={!me.canAssign || !editable}
                    ...
                    className={`inline-flex items-center gap-1 rounded-full border py-0.5 pl-0.5 pr-2 text-xs font-medium disabled:opacity-60 ${on ? "border-neutral-900 bg-indigo-500 text-white" : "border-neutral-300 text-neutral-600 hover:bg-neutral-50"}`}>
  ...
  return (
    <div className="rounded-xl border border-neutral-200 p-3">
      ...
      <form className="mt-2 flex gap-2" onSubmit={(e) => { e.preventDefault(); if (body.trim()) { run(() => addTaskComment(task.id, body)); setBody(""); } }}>
        ...
        <button disabled={pending} className="rounded-lg bg-indigo-500 px-3 py-1.5 text-sm font-medium text-white hover:bg-indigo-400 disabled:opacity-50">등록</button>
      </form>
    </div>
  );
}
